...
final class AlarmStore: ObservableObject {
    ...
    func addAlarm(label: String? = nil, hour: Int = 8, minute: Int = 0, repeatDays: Set<Weekday> = []) {
        let item = AlarmItem(label: label ?? "闹钟", hour: hour, minute: minute, repeatDays: repeatDays)
        items.append(item)
    }
