...
def create_tray_icon():
    ...
    try:
        ...
        # 使用系统默认图标
        hicon = win32gui.LoadIcon(0, win32con.IDI_WINLOGO)
...
def set_window_style(hwnd):
    ...
    win32gui.SetWindowPos(hwnd, 0, 0, 0, 0, 0, win32con.SWP_NOMOVE | win32con.SWP_NOSIZE | win32con.SWP_NOACTIVATE | win32con.SWP_FRAMECHANGED)
...
'''
打包指令：
pyinstaller --onefile --windowed --name PyIsland --icon=icon/pyisland.ico --add-data "island.html;." --add-data "icon;icon" --hidden-import=webview --hidden-import=bleak main.py
'''